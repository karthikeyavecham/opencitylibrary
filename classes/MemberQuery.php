...
class MemberQuery extends Query {
	...
	*/
	function insert($mbr) {
		$sql = $this->mkSQL("insert into member "
				. "(mbrid, create_dt, last_change_dt, "
		...
		$this->updateBarCodeNumber($mbrid);
	...
	****************************************************************************
	*/
	function updateBarCodeNumber($mbrid)
	{
		$sql=$this->mkSQL("update member set barcode_nmbr=%Q where mbrid=%N",$mbrid,$mbrid);
		$this->exec($sql);
	}
